...
rownames(ASVsr_20) <- log(((10^subset(Tau_Seq, Day == 20)$Tau)/60), 10)
...
print(mean(niche.overlap(as.data.frame(OTU_t), method = "czech")))

OTU_t <- OTUsREL_20[,1:1000]
x <- 1
while(x < 100){
  for(col in 1:ncol(OTU_t)){
    OTU_t[,col] <- sample(OTU_t[,col])
  }
  print(mean(niche.overlap(as.data.frame(OTU_t), method = "pianka")))
  niche.overlap <- c(niche.overlap, mean(niche.overlap(as.data.frame(OTU_t), method = "pianka")))
  x = x + 1
}
```
