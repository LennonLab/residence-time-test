...
Read in data
```{r}
PCR <- read.csv("data/RTLC/PCR_check.csv", header = TRUE)
...
Y <- subset(PCR, PCR$Y.N == "Y")
Y <- Y$Tau

N <- subset(PCR, PCR$Y.N == "N")
N <- N$Tau

t.test(Y, N)

hist(Y)
PCR$Date <- as.character(PCR$Date)

PCR_plot <- ggplot(PCR, aes(x = Y.N, y = Tau))+
  geom_boxplot()+
  geom_jitter()

PCR_plot

PCR_plot <- ggplot(PCR, aes(x = Date, y = Tau))+
  geom_boxplot()+
  geom_jitter()
